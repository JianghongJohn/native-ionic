...
@interface CarOrderGuaranteeController ()<UITableViewDelegate,UITableViewDataSource>

@property(nonatomic,strong)UITableView *tableView;

@end

@implementation CarOrderGuaranteeController
...
- (void)viewDidLoad {
    ...
    if (!self.viewModel) {
        ...
        self.viewModel = [[CarOrderAssureViewModel alloc] init];
    }
...
-(void)_creatSubmitButton{
    ...
    [button setTitle:@"保存担保人信息" forState:0];
 ...
 */
-(void)_submit{
    [[NSNotificationCenter defaultCenter]postNotificationName:kSubmitCarOrder object:self.viewModel.asureArray];
