...
-(void)_rightAction{
    ...
    [self.viewModel.dataList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        message = [message stringByAppendingString:[self CheckDataByModel:model message:@"" person:[NSString stringWithFormat:@"%@%lu：",@"配偶",idx+1]]];
    ...
    [JH_NetWorking requestData:[kBaseUrlStr_Local1 stringByAppendingString:kaddspouse] HTTPMethod:HttpMethodPost  showHud:YES  params:[JH_NetWorking addKeyAndUIdForRequest:finalData] completionHandle:^(id result) {
...
-(void)_deleteCellAction:(UIButton *)btn{
    ...
    NSString *title = [NSString stringWithFormat:@"确认删除配偶%@",model.name?model.name:@""];
