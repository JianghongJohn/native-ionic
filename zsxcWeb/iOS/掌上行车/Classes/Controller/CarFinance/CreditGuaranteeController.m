...
- (void)viewDidLoad {
    ...
    if (!self.viewModel) {
        ...
        self.viewModel.baseNumber = _baseNumber;
...
-(void)_rightAction{
    ...
    [self.viewModel.dataList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        message = [message stringByAppendingString:[self CheckDataByModel:model message:@"" person:[NSString stringWithFormat:@"%@%lu：",@"担保人",idx+1]]];
...
-(UITableView *)tableView{
    if (!_tableView) {
        _tableView = ({
            UITableView *table = [[UITableView alloc] initWithFrame:CGRectMake(0, headerHeight, self.view.bounds.size.width, self.view.bounds.size.height-kNavHeight-thisMenuHeight-headerHeight) style:UITableViewStyleGrouped];
            if (_ordernumber) {
                table.frame = CGRectMake(0, kNavHeight+headerHeight, self.view.bounds.size.width, self.view.bounds.size.height-kNavHeight-thisMenuHeight-headerHeight);
            }
            table.delegate     = self;
