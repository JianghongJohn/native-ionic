...
-(UITableViewCell *)JH_setUpTableViewCell:(NSIndexPath *)indexPath WithHandle:(void(^)())completionBlock{
    ...
    if (indexPath.section==0) {//车辆信息
        ...
        //1.首先判断是否为空
            ...
            [JH_NetWorking requestData:[kBaseUrlStr_Local1 stringByAppendingString:kcalculateageinfo] HTTPMethod:HttpMethodPost showHud:YES params:[JH_NetWorking addKeyAndUIdForRequest:requestData] completionHandle:^(id result) {
                if ([result[@"code"]isEqualToString:kSuccessCode]) {
                    ...
                    for (NSString *key in [priceinfo allKeys]) {
                        [self.car setValue:priceinfo[key] forKey:key];
                    }
                    
                    [[NSNotificationCenter defaultCenter]postNotificationName:kCalculateCar object:nil];
        ...
        WeakSelf
        TypicatTextView *view = [[TypicatTextView alloc] initWithFrame:CGRectMake(10, 10, kScreen_Width-20, 50) withType:TypicalViewText withTitle:@"描述：" withText:weakSelf.car.remark withAddInfo:@"" titleArray:@[] withNecessary:YES];
        view.typical = @"remark";
        [view setInputBlock:^(NSString *text,NSString *typical){
            weakSelf.car.remark = text;
            completionBlock();
        }];
